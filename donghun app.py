...
client = MongoClient('mongodb+srv://test:sparta@cluster0.0qyin5k.mongodb.net/cluster0?retryWrites=true&w=majority')
...
def detail(num):
    ...
    post = db.find_my_pet.find_one({'num': num}, {'_id': False})

    # 쿠키에 있는 유저의 아이디와 board에 있는 게시물의 id가 같으면 Ture
    status = post["nickname"] == user_info['nickname']

    return render_template("detail.html", num=num, bool_sign_in=bool_sign_in,
                           user_info=user_info, post=post, status=status)
